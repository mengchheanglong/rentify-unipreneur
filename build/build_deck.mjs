...
// Appendix 11. Competitor evidence
{
  ...
  const tb=s.tables.add({rows:5,columns:3,left:72,top:188,width:1136,height:376,values:rows,columnWidths:[205,465,466]});
  tb.styleOptions={headerRow:true,bandedRows:true};tb.borders.assign({style:'solid',fill:C.line,width:1});
  tb.cells.block({row:0,column:0,rowCount:5,columnCount:3}).assign({textStyle:{fontSize:22,typeface:'Arial',color:C.ink},margins:{left:14,right:14,top:14,bottom:10}});
  for(let c=0;c<3;c++)tb.getCell(0,c).fill=C.navy;
  tb.cells.block({row:0,column:0,rowCount:1,columnCount:3}).assign({textStyle:{fontSize:22,typeface:'Arial',color:C.white,bold:true}});
...
const result=await finalizePresentation({
  ...
  requiredNativeTableOwnerSlides:[11],requiredNativeChartOwnerSlides:[],
  fontPolicy:{basis:'design',families:fonts},verifyArtifactToolImport:true,
  receiptPath:path.join(stage,'Rentify_UniPreneur_Pitch.validation-v5-team-portraits-cropped.json')
});
