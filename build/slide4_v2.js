// Slide 4 ("add once, live everywhere") as a single-slide draft for review.
...
const OUT = path.join(ROOT, 'output', 'drafts', 'Rentify_slide04_add-once_v2.pptx');
...
(async () => {
  ...
  T('Add a product once. It’s live everywhere.', 0.6, 0.35, 12.13, 0.7, { size: 32, bold: true, align: 'center' });
  ...
  const tx = cx + ch + 0.05;
  T('ENTERED ONCE', tx, cy + 0.2, 3, 0.3, { size: 11, bold: true, color: C.teal, cs: 2 });
  T('Yes I Am The Queen', tx, cy + 0.5, cw - ch - 0.2, 0.45, { size: 17, bold: true });
  T([{ text: '$36.00', options: { bold: true, color: C.ink } }, { text: '   ·   Stock 40', options: { bold: true, color: C.blue } }],
    tx, cy + 1.0, 3, 0.4, { size: 17 });

  // Three real channel screens
  const sw = 3.95, sgap = 0.24, sx0 = (13.333 - (3 * sw + 2 * sgap)) / 2, sy = 3.5, bar = 0.26;
  const screens = [
    ['v6/r2/c-sf-before.png', 1760, 990, 'Own storefront', C.blue, 'Globe'],
    ['v6/r2/c-mp-before.png', 2160, 1215, 'Marketplace', C.orange, 'ShoppingBag'],
    ['v6/r2/c-pos-search.png', 1760, 990, 'POS at the counter', C.teal, 'Store'],
  ];
  for (let i = 0; i < 3; i++) {
    const [file, pw, ph, label, col, ic] = screens[i];
    ...
    s.addText([{ text: '$36', options: { bold: true, color: C.ink } }, { text: '  ·  Stock 40', options: { bold: true, color: col } }], {
  ...
  T('Real screens from Rentify’s working development build (demo store: Aura Botanicals).', 0.6, 6.95, 11.4, 0.3,
    { size: 10, color: C.muted, valign: 'top' });

  s.addNotes(`[1:05–1:25]
Here is how Rentify works. The merchant adds a product once — name, price and stock. It appears immediately on their own storefront, in the shared marketplace and in their POS at the counter. Same price, same stock, everywhere. No copying, no re-listing.`);
